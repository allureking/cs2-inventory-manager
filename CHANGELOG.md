...
### 新增 / Added
- **大盘指数自动填入**：收益追踪表的「大盘指数」不再需要每天手动敲。每日 00:03 PT 自动
  从 SteamDT 开放平台取值，并**自动补齐此前所有空缺**（生产上 2026-07-22 起已连空 10 天）
  / **Market index auto-fill** with automatic backfill of past gaps

  - 数据源：`POST /open/cs2/broad/v1/kline {"type":1}`，一次返回 2160 根小时线 ≈ 90 天。
    选它而非只给当前值的 `/broad/v1/index`，是因为同一次调用既填当天、也补历史——
    任务哪天挂了、服务停了几天，下次跑自动追平，不需要人工补录
  - **口径是反推出来的，不是猜的**：拿生产库 78 天手工值对小时线做全组合比对
    （时区 × open/close × 24 小时），最优解是 **PT 00:00 那根的收盘价**（平均绝对误差
    1.53，其中 6 天完全精确匹配、偏差中位数 -0.06 无系统性偏移）。该时刻恰与
    `snapshot_daily` 写库存价值同刻，两者天然同步对应
  - **只填空不覆盖**：235 天手工数据原样保留；需要覆盖须显式传 `overwrite=true`
  - **不新建行**：只更新已存在的日期，指数依附于「那天有库存数据」
  - 手动触发：`POST /api/tracker/market-index/sync?days=90`

  - **抗列序漂移**：K 线是没有字段名的纯数组，上游一旦调整列序，「时间戳(约 17 亿)
    被当成指数写进去」这种事光靠正数校验拦不住。两端都加了合理性带（时间戳要像
    时间戳、指数要像指数），列序变了就整批拒绝并告警，而不是静默写错
  - 限流（4005）有界重试 3 次；失败到底也只是今天没填上，明天那次会连着补回来

### 测试 / Tests
- 472 用例（+16）。6 次破坏性验证全部精确打红：取 open 而非 close / 取错小时 /
  去掉「只填空」保护 / 用固定 -7 偏移替代时区库（DST 切换日会错行）/ 拉取失败改抛异常 /
  去掉列序漂移防护
